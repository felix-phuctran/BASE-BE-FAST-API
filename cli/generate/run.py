...
def main():
    ...
    parser.add_argument(
        "module_name", 
        help="Module name in snake_case (e.g. membership_package)"
    )
...
def generate_files(module_name: str):
    """
    Main function to:
      - Build context (name_sing, model, names, models).
    ...
    # 2) Singularize
    name_sing = singularize(snake_name)          # e.g. "membership_package" (if it was "membership_packages" => "membership_package")
    # 3) Create PascalCase
    model = to_camel(name_sing)                  # e.g. "MembershipPackage"
    # 4) Create a plural form
    names = f"{name_sing}s"                      # e.g. "membership_packages"
    models = to_camel(names)                     # e.g. "MembershipPackages"
    ...
    context = {
        ...
        "models": models,        # "MembershipPackages"
    }
    ...
    template_output_map = [
        ...
        ("api/v1/endpoints/{{name_sing}}.hbs",
         "api/v1/endpoints/{{name_sing}}.py"),
    ]
